object frmHerancaClasse: TfrmHerancaClasse
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Heran'#231'a de classes'
  ClientHeight = 138
  ClientWidth = 396
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object btnClasseAnimal: TBitBtn
    Left = 135
    Top = 8
    Width = 121
    Height = 49
    Caption = 'Classe Animal'
    TabOrder = 0
    OnClick = btnClasseAnimalClick
  end
  object btnClasseCachorro: TBitBtn
    Left = 8
    Top = 80
    Width = 121
    Height = 42
    Caption = 'Classe Cachorro'
    TabOrder = 1
    OnClick = btnClasseCachorroClick
  end
  object btnClassePato: TBitBtn
    Left = 135
    Top = 80
    Width = 121
    Height = 42
    Caption = 'Classe Pato'
    TabOrder = 2
    OnClick = btnClassePatoClick
  end
  object btnClasseGato: TBitBtn
    Left = 262
    Top = 80
    Width = 121
    Height = 42
    Caption = 'Classe Gato'
    TabOrder = 3
    OnClick = btnClasseGatoClick
  end
  object btnFonte1: TBitBtn
    Left = 352
    Top = 5
    Width = 39
    Height = 41
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    TabOrder = 4
    OnClick = btnFonte1Click
  end
end
