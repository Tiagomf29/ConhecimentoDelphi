object frmCopiaObjetos: TfrmCopiaObjetos
  Left = 0
  Top = 0
  Caption = 'C'#243'pia de um objeto para outro objeto'
  ClientHeight = 294
  ClientWidth = 367
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object btnCopiaObjeto: TButton
    Left = 0
    Top = 8
    Width = 337
    Height = 25
    Caption = 'Copiar dados do objeto e listar no memo abaixo'
    TabOrder = 0
    OnClick = btnCopiaObjetoClick
  end
  object mmo1: TMemo
    Left = 0
    Top = 39
    Width = 362
    Height = 250
    TabOrder = 1
  end
  object btnFonte1: TBitBtn
    Left = 340
    Top = 8
    Width = 22
    Height = 25
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    TabOrder = 2
    OnClick = btnFonte1Click
  end
end
