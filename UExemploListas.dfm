object frmExemploListas: TfrmExemploListas
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Exemplo Cria'#231#227'o de listas'
  ClientHeight = 324
  ClientWidth = 474
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object btnCriaLista: TButton
    Left = 8
    Top = 8
    Width = 185
    Height = 33
    Caption = 'Criar lista de Objetos (TObjectList)'
    TabOrder = 0
    OnClick = btnCriaListaClick
  end
  object mmo1: TMemo
    Left = 8
    Top = 47
    Width = 209
    Height = 274
    TabOrder = 1
  end
  object btnCriarlista2: TButton
    Left = 255
    Top = 8
    Width = 185
    Height = 33
    Caption = 'Criar lista de Objetos (TList)'
    TabOrder = 2
    OnClick = btnCriarlista2Click
  end
  object mmo2: TMemo
    Left = 255
    Top = 47
    Width = 210
    Height = 274
    TabOrder = 3
  end
  object btnFonte1: TBitBtn
    Left = 196
    Top = 8
    Width = 22
    Height = 33
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    TabOrder = 4
    OnClick = btnFonte1Click
  end
  object btnFonte2: TBitBtn
    Left = 443
    Top = 8
    Width = 22
    Height = 33
    Margins.Left = 0
    Margins.Top = 0
    Margins.Right = 0
    Margins.Bottom = 0
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    TabOrder = 5
    OnClick = btnFonte2Click
  end
  object mmo3: TMemo
    Left = 503
    Top = 42
    Width = 210
    Height = 274
    TabOrder = 6
  end
end
